...
object Main {
  def main(args: Array[String]) {
    ...
    val count = NewsService.getNumberOfNews(enrichedDataset)
    ...
    val climateNews = spark.sql("SELECT * FROM news WHERE containsWordGlobalWarming = true")
    climateNews.show()
    ...
    val climateNewsTyped = enrichedDataset.filter(_.containsWordGlobalWarming)
    climateNewsTyped.show()}
